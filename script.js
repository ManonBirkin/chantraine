...
(function () {
  ...
  ready(() => {
    ...
    ===================================================== */
    const revealElements = document.querySelectorAll('.reveal, .reveal-stagger, .line-reveal, .split-heading, .scale-in, .slide-in-left, .slide-in-right, .video-wrapper');
    ...
    function updateTransform(animate = true) {
      ...
      // Update pan cursor
      if (scale > 1) {
        lbImg.classList.add("can-pan");
      } else {
        lbImg.classList.remove("can-pan");
      }

      // Update button states
      lbZoomBtns.forEach(btn => {
        if (btn.dataset.action === "out") {
          btn.disabled = scale <= MIN_SCALE;
        } else if (btn.dataset.action === "in") {
          btn.disabled = scale >= MAX_SCALE;
        }
      });
    }
    ...
    function clampPan() {
      ...
      // Dimensions réelles de l'image à l'échelle courante
      const scaledWidth = lbImg.naturalWidth * (imgRect.width / lbImg.naturalWidth) * scale / scale;
      const scaledHeight = lbImg.naturalHeight * (imgRect.height / lbImg.naturalHeight) * scale / scale;

      // On récupère la taille affichée de base (à scale=1)
      const baseWidth = imgRect.width / scale;
      const baseHeight = imgRect.height / scale;

      // Calcul de combien l'image dépasse
    ...
    function zoomTo(newScale, centerX = null, centerY = null) {
      ...
      // Si on revient à 1, reset la position
      if (scale === 1) {
        translateX = 0;
        translateY = 0;
      } else if (centerX !== null && centerY !== null) {
        // Zoom centré sur un point (pour molette)
        const viewportRect = lbViewport.getBoundingClientRect();
        const imgRect = lbImg.getBoundingClientRect();

        // Position du centre de zoom relative au centre de l'image
        const imgCenterX = imgRect.left + imgRect.width / 2;
        const imgCenterY = imgRect.top + imgRect.height / 2;
        const offsetX = centerX - imgCenterX;
        const offsetY = centerY - imgCenterY;

        // Ajuster le translate pour garder le point sous le curseur
    ...
    // Clic sur overlay (hors image)
    lb.addEventListener("click", (e) => {
      if (e.target === lb || e.target === lbViewport) {
        closeLightbox();
      }
    });
    ...
    // Double-clic sur image : toggle 1x ↔ 2x
